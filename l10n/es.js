OC.L10N.register(
    "integration_giphy",
    {
    "GIF picker (by Giphy)" : "Seleccionador de GIFs (por Giphy)",
    "GIF not found" : "No se encontró el GIF",
    "Giphy GIFs" : "GIFs de Giphy",
    "Bad HTTP method" : "Método HTTP erróneo",
    "Bad credentials" : "Credenciales erróneas",
    "Connected accounts" : "Cuentas conectadas",
    "Giphy integration" : "Integración Giphy",
    "Integration of Giphy" : "Integración de Giphy",
    "Giphy integration provides a unified search provider for GIFs.\n\nIt also provides a link reference provider to render links to GIFs and make it possible to search for GIFs in Talk, Text and potentially anywhere in Nextcloud.\n\nTo use SmartPicker, start typing with `/giphy`." : "La integración con Giphy proporciona un proveedor de búsqueda unificada de GIFs.\n\nAdemás proporciona un proveedor de referencias de enlaces para generar enlaces a GIFs y posibilitar la búsqueda de GIFs en Talk, Text y potencialmente en cualquier lugar dentro de Nextcloud.\n\nPara usar el selector inteligente, comience a escribir usando `/giphy`.",
    "G - Level 1" : "G - Nivel 1",
    "PG - Level 2" : "PG - Nivel 2",
    "PG 13 - Level 3" : "PG 13 - Nivel 3",
    "R - Level 4" : "R - Nivel 4",
    "Giphy admin options saved" : "Las opciones de administrador Giphy fueron guardadas",
    "Failed to save Giphy admin options" : "Fallo al guardar las opciones de administrador Giphy",
    "Giphy API key" : "Clave API de Giphy",
    "You can create a Giphy API key in your Giphy developer settings." : "Puede crear una clave API de Giphy en sus ajustes de desarrollador Giphy.",
    "Giphy developer settings" : "Ajustes de desarrollador de Giphy",
    "Rating filter" : "Filtro de calificación",
    "Enable search provider for GIFs" : "Habilitar el proveedor de búsqueda para GIFs",
    "Enable Giphy link previews" : "Habilitar la vista previa de los enlaces de Giphy",
    "Giphy options saved" : "Opciones Giphy guardadas",
    "Failed to save Giphy options" : "Error al guardar las opciones de Giphy",
    "Searching gifs has been disabled by your administrator" : "La búsqueda de GIFs ha sido desactivada por el administrador",
    "Gif link previews have been disabled by your administrator" : "La previsualización de enlaces a GIFs ha sido desactivada por el administrador",
    "Loading GIF" : "Cargando GIF",
    "Search GIFs" : "Buscar GIFs",
    "Powered by Giphy" : "Desarrollado por Giphy",
    "Too many requests to Giphy. Please contact your administrator." : "Demasiadas peticiones a Giphy. Por favor contacta con el administrador",
    "Failed to load GIFs" : "Error al cargar GIFs",
    "Giphy GIF picker" : "Selector GIF Giphy",
    "Searching..." : "Buscando ...",
    "No results" : "Sin resultados",
    "No more GIFs" : "No hay más GIFs",
    "Fold all Giphy GIFs" : "Doblar todos los GIFs de Giphy",
    "Unfold all Giphy GIFs" : "Desdoblar todos los GIFs de Giphy",
    "GIFs are disabled" : "Los GIFs están deshabilitados"
},
"nplurals=3; plural=n == 1 ? 0 : n != 0 && n % 1000000 == 0 ? 1 : 2;");
